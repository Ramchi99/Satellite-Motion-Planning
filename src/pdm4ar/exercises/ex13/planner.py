...
class SolverParameters:
    ...
    stop_crit: float = 1e-7  # Stopping criteria constant
    eps_r: float = 1e-7
...
class SatellitePlanner:
    ...
    def initial_guess(self) -> tuple[NDArray, NDArray, NDArray]:
        ...
        number = 3
        t_f_guess = number * np.sqrt(m * norm(X0[:2] - Xf[:2]) / F_max)
        p = np.array([t_f_guess])
        print("Initial guess for final time:", t_f_guess)
